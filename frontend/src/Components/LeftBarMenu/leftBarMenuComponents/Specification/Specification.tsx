...
import { SpecProperties } from './SpecProperties/SpecProperties'
...
import {
  SPECIFICATION_HISTORY_PATH,
  SPECIFICATION_LINK_PATH,
  SPECIFICATION_PATH,
  TASK_LINK_PATH,
...
export const Specification: React.FC<SpecProps> = ({ status, setStatus }) => {
  const settingsValue = ['Атрибуты документа', 'Путь документа', 'Замечания', 'Спецификация']
  ...
  return (
    <div className={style.component}>
      ...
      <Switch>
        <Route exact path={SPECIFICATION_PATH}>
          <SpecProperties checkbox={checkbox} setStatus={setStatus} />
        </Route>
        <Route path={SPECIFICATION_LINK_PATH}>
          <div>Link</div>
        </Route>
        <Route path={SPECIFICATION_HISTORY_PATH}>
          <div>History</div>
        </Route>
      </Switch>
    </div>
